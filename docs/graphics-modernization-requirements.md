...
> **Phase 0.16 supersession — 2026-07-30:** Phase 0.16 preserves the complete Phase 0.15 cover projection and every Phase 0.13/0.14 active-match card and coin behavior. It supersedes only the earlier immediately-settled active-match hand presentation at match entry. While Modern is effective, every nonempty one- through five-card current hand initially occupies one pile at its existing first/top slot. The current last card—the card destined for the bottommost occupied position—retains the highest normal render order and is therefore visible on top of the pile. Only the actual Legacy left-panel opening completion, after the cover parent is hidden, publishes a plain observation event that may release both piles. For a five-card hand, cards `4`, `3`, `2`, and `1` then spread downward in that order through one deterministic mirrored 3D lift over a 785-millisecond batch while card `0` remains at the top anchor. Shorter resumed hands apply the same last-current-through-second rule independently. Canonical hand descriptions, Legacy nodes, game/controller state, turn state, legal targets, and requests remain unchanged. The animation locks Modern card input until exact settlement, snaps under reduced motion, cancels safely at lifecycle boundaries, and intentionally settles without replay if the complete Modern presentation is not ready at the authoritative cover-open instant. The corrected Phase 0.16 delivery identity is `0.185.1-match-hand-fan.2`; the Phase 0.15 cover component retains its own historical identity `0.185.1-game-cover-hinge.1`.

> **Phase 0.16 direction correction — 2026-07-30:** The rejected `0.185.1-match-hand-fan.1` implementation conflated the card visible on top of the pile with the pile's stationary spatial anchor. It placed each pile at the last/bottom destination and moved cards upward from beneath it. That behavior does not represent a person placing a stack at the top of a hand and pushing it open. Version `0.185.1-match-hand-fan.2` supersedes that geometry: the first/current top slot is the pile anchor, index `0` is stationary, the last current card remains initially visible on top, and current indices `n - 1` through `1` move downward into the compacted current hand positions.
...
| **Match-hand stack** | One Phase 0.16 transient pile per nonempty side at that current hand's existing first/top-slot center. Every current card shares that center and a neutral table pose. The last current index `n - 1`, which will occupy the bottommost current position after expansion, retains the highest normal hand render order and is therefore the visible top card; index `0` is the spatial anchor. |
| **Match-hand fan** | The deterministic Phase 0.16 renderer-local expansion that leaves hand index `0` fixed and moves current indices `n - 1` down through `1` from the top pile into their unchanged compacted vertical positions with mirrored lateral arc, lift, pitch, yaw, and roll. It is presentation only and is unrelated to legal-card selection or gameplay hand mutation. |
...
| DEC-092 | The Phase 0.15 cover component retains cache identity `0.185.1-game-cover-hinge.1` in its source registration, cover facade/ABI, cover DOM metadata, diagnostics, and component contracts. That value was also the composite generated-artifact, coordinator-URL, and deployment identity while Phase 0.15 was current. A later phase may supersede only those singular outer delivery identities while continuing to expose the cover component identity; corrected Phase 0.16 does so with `0.185.1-match-hand-fan.2`. Three.js remains `0.185.1` (`r185`), and Phase 0.14's `0.185.1-match-turn-coin.1` remains historical evidence. |
...
| DEC-094 | Each nonempty side's pile is anchored at its current first/top card center: player `(86.5, 91)` and opponent `(608.5, 91)`, regardless of whether that compacted resumed hand contains one, two, three, four, or five cards. Index `0` remains stationary. The last current index `n - 1` retains the highest normal render order, paints above the other current cards in the pile, and moves first and farthest downward; no new random stack offset, curved-table skew, camera fan, or canonical-coordinate mutation is permitted. |
...
| DEC-096 | One renderer-neutral deterministic planner and sampler owns the fan. Moving cards use 620-millisecond cubic-out travel with 55-millisecond reveal staggering. For a five-card hand, reveal order is `4`, `3`, `2`, `1` and total batch duration is 785 milliseconds; index `0` stays at the top anchor. For any shorter hand, the current last index moves first and the sequence continues downward through index `1`. Both sides share vertical timing and mirror only bounded lateral, yaw, and roll signs. No random value or frame-to-frame integration is used. |
...
| DEC-100 | The corrected Phase 0.16 generated Modern artifact retains Three.js `0.185.1` (`r185`) and uses cache identity `0.185.1-match-hand-fan.2`. Source, generated bundle, facade/ABI, coordinator URL, diagnostics, static/browser contracts, deployment artifact, and this document must agree. The rejected upward-spreading artifact remains historical as `0.185.1-match-hand-fan.1`; the independently delivered Phase 0.15 cover retains `0.185.1-game-cover-hinge.1` as its component identity. |
...
With Modern effective, the covered active-match surface prepares each nonempty player and opponent hand as a compact pile at its existing first/top position. The current card destined for the bottommost occupied position is visibly on top of each pile. When the game box has fully opened and the existing Legacy cover removes its board shield, that visible card moves first and farthest downward, revealing the next card beneath it; the sequence continues until the current one- through five-card hand occupies its familiar compacted vertical positions. Index `0` remains fixed at the top while the other current cards spread downward through a restrained mirrored 3D lift.
...
62. Each nonempty side piles independently at its first current card destination, preserves normal hand render order so its last current card is initially visually topmost, keeps index `0` stationary, and moves indices `n - 1` through `1` downward into the compacted current hand positions.
...
**FR-GAME-COVER-025** — The cover source registration, cover facade/ABI, cover DOM/canvas metadata, runtime diagnostics, and cover contracts must retain component cache identity `0.185.1-game-cover-hinge.1` and Three.js `0.185.1` (`r185`). While Phase 0.15 was current, the composite generated artifact, coordinator URL, and deployment artifact shared that value. Beginning with corrected Phase 0.16, those singular outer delivery identities are superseded by `0.185.1-match-hand-fan.2`; the embedded cover component must continue to report `0.185.1-game-cover-hinge.1`. `0.185.1-match-turn-coin.1` remains the historical Phase 0.14 identity.
...
**FR-MATCH-HAND-ENTRANCE-002** — Before the accepted cover-open settlement, every current card on a nonempty side must render at that side's first/current top-card center with zero depth and zero local rotation. For every one- through five-card compacted hand those centers are player `(86.5, 91)` and opponent `(608.5, 91)`. Index `0` is the stationary spatial anchor. The current last index `n - 1` must retain the highest normal hand face/body render order and therefore appear on top of each pile.
...
**FR-MATCH-HAND-ENTRANCE-007** — Planning and sampling must be implemented in one renderer-neutral deterministic module with no DOM, jQuery, Raphael, Three.js, storage, game-controller, network, clock, frame scheduler, or random dependency. It accepts the current plain hand rectangles, derives one independent first-card/top anchor per nonempty side, and returns immutable plain plans and sampled poses. Production, facade consumers, and pure tests must use the same planner and sampler.

**FR-MATCH-HAND-ENTRANCE-008** — Index `0` remains stationary at the first/top anchor. For a five-card side, indices `4`, `3`, `2`, and `1` move downward after delays `0`, `55`, `110`, and `165` milliseconds. For an `n`-card side, current index `n - 1` moves first and the sequence continues down through index `1`; the batch durations for one through five cards are respectively `0`, `620`, `675`, `730`, and `785` milliseconds. Each moving card lasts 620 milliseconds. Local progress uses cubic-out `1 - (1 - t)^3`; completion normalizes every position, depth, pitch, yaw, roll, scale, and render order exactly to its canonical hand pose.
...
**FR-MATCH-HAND-ENTRANCE-016** — The corrected Phase 0.16 source and generated bundle must retain Three.js `0.185.1` (`r185`) and share cache identity `0.185.1-match-hand-fan.2`. Source registration, generated artifact, facade/ABI, coordinator URL, diagnostics, pure/static/browser contracts, deployment artifact, and this document must agree. The rejected upward-spreading artifact is historical as `0.185.1-match-hand-fan.1`; the parallel cover component retains `0.185.1-game-cover-hinge.1` as its Phase 0.15 identity.
...
1. The Modern active-match surface prepares each nonempty current hand as one compact pile at its first/top slot while the game cover still blocks the board.
2. The current last card, which will occupy the bottommost current position after expansion, is visibly on top of each pile.
3. The existing Legacy cover completes its real opening animation and hides the full-stage blocker.
4. The visible last card moves first and farthest downward, then the cards beneath it follow in reverse-index order until both hands occupy the exact compacted vertical positions already accepted in Phase 0.10.
...
| Side | Index 0 / pile anchor | Index 1 | Index 2 | Index 3 | Index 4 |
|---|---:|---:|---:|---:|---:|
| Player x | 86.5 | 86.5 | 86.5 | 86.5 | 86.5 |
| Opponent x | 608.5 | 608.5 | 608.5 | 608.5 | 608.5 |
| y | 91 | 146 | 201 | 256 | 311 |

The waiting pile always uses the first current card's center rather than the current count's last destination. A one-, two-, three-, four-, or five-card compacted side therefore piles at its first/top destination at y center `91`; a one-card side is already terminal, and an empty side contributes no plan. Player and opponent counts are independent.

Every waiting card has:

```text
screen center = that side's first current destination
depth         = 0
rotation X/Y/Z = 0
projected scale = 1
```

Normal existing hand render order is preserved. Index `4` is highest for a five-card hand and consequently covers indices `0` through `3` in the pile. This is the user's requested “bottommost card on top”: it describes the card's eventual vertical destination and pile paint order. It is deliberately separate from the spatial anchor: index `0` holds the top position, while visible index `4` moves first and farthest downward.
...
Each side is planned independently from its current first/top-card source to its unchanged compacted destinations. Index `0` remains stationary. The visible last current card moves first, revealing the next card beneath it, and release order continues in descending index order through index `1`:

```text
index 4 -> delay   0 ms
index 3 -> delay  55 ms
index 2 -> delay 110 ms
index 1 -> delay 165 ms
```
...
Before cover settlement, a ready normal five-card Modern fixture shows all player centers at `(86.5, 91)` and all opponent centers at `(608.5, 91)`, with zero depth and rotation. Canonical diagnostic rectangles still report y centers `91`, `146`, `201`, `256`, and `311`. Index `0` is stationary, while index `4` has the highest ordinary face and body render order on both sides. One- through five-card and asymmetric fixtures all use the same y-`91` top anchor without inventing absent cards. No card input handler is attached and clicks produce no hold, placement, semantic action, or request.
...
Pure tests cover zero through five cards per side, asymmetric counts, independent top anchors, immutability, input non-mutation, invalid values, exact delays and one- through five-card batch durations, and no fewer than one dense sample per millisecond. At global 310 milliseconds, the farthest index-4 cards in a five-card fixture are at their local midpoint at y center `283.5` and reach depth `18`, lateral offsets `-4`/`+4`, pitch `-4.5°`, yaw `-2°`/`+2°`, and roll `-1.5°`/`+1.5°`. Every sample is finite and bounded, vertical travel is monotonically downward, both sides mirror exactly, repeated sampling is history-independent, and final poses equal canonical anchors exactly.

**AC-P016-004 — Controlled-clock surface integration**

A controlled browser clock proves the ready surface remains in two top-anchored piles until an accepted matching settlement, starts one 785-millisecond batch, keeps index `0` fixed, moves indices `4` through `1` downward in staggered order with nonzero 3D depth and rotation, owns at most one entrance frame, blocks input throughout, and ends with ten exact canonical poses, zero entrance frames, one completed sequence, and restored Phase 0.13 input. Additional actual-surface coverage proves asymmetric resumed hands use only their current one- through five-card counts and preserve opponent-card secrecy.
...
- controlled browser-clock coverage for y-`91` waiting piles, exact last-through-second order and midpoint 3D pose, stationary index `0`, asymmetric partial hands, input lock, one pending frame, exact terminal hands, opponent secrecy, and zero request;
...
- actual-size normal-speed review proving that both sides begin as compact piles at the top hand positions, the eventual bottom card is initially visible on top and moves first and farthest downward, each following card is revealed in sequence, the motion reads on one flat table rather than a curved surface, and the final layout is the accepted compacted Phase 0.10 layout;
...
- a generated Modern artifact matching reviewed source and pinned Three.js `0.185.1` with cache identity `0.185.1-match-hand-fan.2`.
...
- Before release, each nonempty current side forms one exact flat pile at its first/top-card destination; index `0` is the spatial anchor and the current last card paints on top.
...
- Index `0` remains stationary; current indices `n - 1` through `1` spread downward in order with bounded mirrored lateral/yaw/roll motion and shared depth/pitch timing, then normalize exactly to the accepted compacted vertical layout.
...
- Diagnostics, source, generated bundle, facade/ABI, loader, tests, deployment, and requirements agree on `0.185.1-match-hand-fan.2`.
...
| `FR-MATCH-HAND-ENTRANCE-*` / `AC-P016-*` | Phase 0.16 | Legacy cover-settlement adapter, graphics coordinator, renderer-neutral hand-entrance module, and Modern active-match surface; Legacy remains cover/continuation authority | Exact independent first/top pile anchors, last-current-card top paint order, hide-before-plain-settlement trigger, guarded ready/settled routing, deterministic one- through five-card cadence with 620/55/785 five-card timing and mirrored bounded 3D samples, stationary index 0, last-through-second downward motion, canonical settlement, input arbitration, reduced motion, lifecycle/stale-frame rejection, zero gameplay/continuation authority, diagnostics, actual-size visual review, and `0.185.1-match-hand-fan.2` identity proof | Yes |
...
- changing Phase 0.16's plain entrance or settlement schemas, current-cover-sequence guard, hide-before-notification trigger, first-current-card pile anchor, last-index-top/index-0-stationary separation, last-through-second downward reveal order, one- through five-card timing, 620-millisecond card duration, 55-millisecond stagger, 785-millisecond five-card batch, cubic-out progression, bounded lift/lateral/rotation values, mirrored-sign policy, exact canonical endpoint, input lock/arbitration, readiness fail-to-settled rule, monotonic terminal sequence, reduced-motion/lifecycle/stale-frame behavior, diagnostics, or `0.185.1-match-hand-fan.2` cache identity without corresponding geometry, timing, visual, authority, lifecycle, and bundle evidence;
